...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "SELECT a.*, f.name as facultyname, c.*, d.* FROM accepted_students a 
        JOIN courses c ON c.course_id = a.course_id
        JOIN departments d ON d.id = c.department_id
        JOIN faculties f ON f.id = d.faculty_id
        WHERE f.id = '$sem' AND a.year = $year AND a.level_of_study = '$level'";
    ...
    <div class="main">
        <h1>Admissions per faculty, level and year</h1>
        <br>
        <p>Filter options</p>
        <form action="" method="POST">
            ...
            <select id="dropdown" class="dropdown" name="level">
